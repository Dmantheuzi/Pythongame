...
pygame.mixer.init(-1) #intialize mixer
...
pygame.display.set_caption("BALL SPORT ") #Name pygame window
...
inst = [font.render(instructions[16], 1, (lightred)),\
        font.render(instructions[17], 1, (blue)),\
        font.render(instructions[18], 1, (lightyellow)),\
        font.render(instructions[19], 1, (hotpink)),\
        font.render(instructions[20], 1, (purple)),\
        font.render(instructions[21], 1, (lightblue))]

#Render game text
onScreen = [font2.render(text[0], 1, (blue)),\
        font2.render(text[1], 1, (red)),\
        font3.render(text[2], 1, (lightblue)),\
        font3.render(text[3], 1, (green)),\
        font3.render(text[4], 1, (hotpink)),\
        font.render(text[5], 1, (yellow)),\
        font.render(text[6], 1, (lightyellow)),\
        font.render(text[7], 1, (lightred)),\
        font.render(text[8], 1, (red)),\
        font.render(text[9], 1, (blue)),\
        font.render(text[10], 1, (hotpink))]

#Initialize all sounds
intro = pygame.mixer.Sound('sms-alert-2-daniel_simon.wav')
whoosh = pygame.mixer.Sound('Woosh-Mark_DiAngelo-4778593.wav')
catch = pygame.mixer.Sound('Ball_Bounce-Popup_Pixels-172648817.wav')
drop = pygame.mixer.Sound('Light Bulb Breaking-SoundBible.com-53066515.wav')
hi = pygame.mixer.Sound('Ta Da-SoundBible.com-1884170640.wav')
tieGame = pygame.mixer.Sound('Sad_Trombone-Joe_Lamb-665429450.wav')

#Function to redraw everything in game window
def redraw_game_window():
    global hiScoreOld
    pygame.Surface.fill(win,(lightyellow))

    #Scores Text
    scores = [font3.render(str(score), 1, (hotpink)),\
              font3.render(str(hiScore), 1, (lightred)),\
              font3.render(str(score2), 1, (purple)),\
                font3.render('{:.2e}'.format(score), 1, (lightyellow)),\
              font3.render('{:.2e}'.format(hiScore), 1, (yellow)),\
              font3.render('{:.2e}'.format(score2), 1, (red))]

    c = 1
    if play == 0:
        #Print instructions before game starts
        for i in range(16):
            j = font3.render(instructions[i], 1, (hotpink))
